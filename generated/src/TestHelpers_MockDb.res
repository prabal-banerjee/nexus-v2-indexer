...
@genType
and entities = {
    @as("VaultEvent") vaultEvent: entityStoreOperations<Entities.VaultEvent.t>,
  }
      ...
      vaultEvent: {
        makeStoreOperatorEntity(
          ...
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.VaultEvent)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.VaultEvent.t,
            >
          ),
          ~getKey=({id}) => id,
        )
